...
function createLocalTestPluginApi(
...
describe("brightdata plugin", () => {
  ...
  it("returns partial failures from batch search without failing the whole tool", async () => {
    ...
    const tool = createBrightDataBatchTools({ pluginConfig: {} } as never).find(
  ...
  it("wraps Bright Data dataset tool payloads as untrusted content", async () => {
    ...
    const tool = createBrightDataWebDataTools({ pluginConfig: {} } as never).find(
  ...
  it("registers the provider and all Bright Data tools", async () => {
    ...
    const api = createLocalTestPluginApi({
      id: "brightdata",
      name: "Bright Data Plugin",
      description:
        "Standalone Bright Data plugin for search, direct scraping, structured data, and browser automation",
      source: "test",
      config: {},
      pluginConfig: {},
      runtime: {} as never,
